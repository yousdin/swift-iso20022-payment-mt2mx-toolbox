...
def get_xpath_and_value(element: ET.Element, path: str = '', strip_space: bool = True) -> List[tuple]:
    """
    Recursively generate (XPath, value) tuples for all elements in the XML tree, stripping namespaces for clarity.
    Returns: List of (xpath, value) tuples
    """
...
def parse_xml_to_xpath_and_value(file_path: str, strip_space: bool = True) -> tuple:
    ...
    - list of (XPath, value, file_path, file_name) tuples for all elements
    ...
    try:
        ...
        results = [(xpath, value, file_path, file_name) for xpath, value in xpaths_and_values]
...
def main():
    ...
    if not xml_files:
        ...
        sys.exit(1)
    all_results = []
    file_metadata = []  # List of (xsd, msgid, fr, to, credt, bizmsgidr, bizsvc) for each file, repeated for each row
    ...
    if all_results:
        ...
        header = "XPath | Value | File | Name"
        print(header)
        for xpath, value, file_path, file_name in all_results:
            if with_labels:
                print(f"{xpath} | Value: {value} | File: {file_path} | Name: {file_name}")
            else:
                print(f"{xpath} | {value} | {file_path} | {file_name}")
        ...
        try:
            with open(output_file, 'w', encoding='utf-8') as f:
                f.write(file_header + '\n')
                for (xpath, value, file_path, file_name), (xsd, msgid, fr, to, credt, bizmsgidr, bizsvc) in zip(all_results, file_metadata):
                    is_empty = str(value == '' or value is None)
                    if with_labels:
                        f.write(f"{xpath} | Value: {value} | File: {file_path} | Name: {file_name} | isEmptyValue: {is_empty} | XSD: {xsd} | MsgId: {msgid} | Fr: {fr} | To: {to} | CreDt: {credt} | BizMsgIdr: {bizmsgidr} | BizSvc: {bizsvc}\n")
                    else:
                        f.write(f"{xpath} | {value} | {file_path} | {file_name} | {is_empty} | {xsd} | {msgid} | {fr} | {to} | {credt} | {bizmsgidr} | {bizsvc}\n")
